...
import { CreateComplaintDto, UpdateComplaintDto, ComplaintFilters } from "../types/complaint";
...
class ComplaintRepository {
  ...
  async findAll(
    ...
    try {
      ...
      return {
        complaints: complaints.map((complaint) => Complaint.fromEntity(complaint)),
        total,
      };
    } catch (error) {
    ...
    try {
      const complaint = await this.prisma.complaint.create({
        data: {
          ...
          createdAt: new Date(),
          updatedAt: new Date(),
        } as Prisma.ComplaintCreateInput,
      });
